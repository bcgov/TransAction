...
namespace TransAction.Data.Repositories
{
    public class UserRepository : RepositoryBase<TraUser>, IUserRepository
    {
        ...
        public IEnumerable<TraUser> GetAll(string name, int page, int pageSize)
        {
            ...
            {
                users = users.Where(x => x.Fname.ToLower().Contains(name.ToLower()) || x.Lname.ToLower().Contains(name.ToLower()));
            }
        ...
        public int Count(string name)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                return userCount.Where(x => x.Fname.ToLower().Contains(name.ToLower()) || x.Lname.ToLower().Contains(name.ToLower())).ToList().Count();
            }
